...
@objcMembers open class AAudioPlayerModule: NSObject {
    
    private let defaultSampleRate:Double = 44100
    private let defaultChannels:AVAudioChannelCount = 2
    open var volume: CGFloat = 0.0
    open var _playIndex: Int = 0
    open var _playerUrl: URL?
    ...
    open var defaultFormat:AVAudioFormat?
    open var musicArr: [Any] = []
    private var audioAsset: AVURLAsset?
    open var reverb:AVAudioUnitReverb = AVAudioUnitReverb()
    open var playerModel: MusicPlayerModel = .sequencePlayerModel
    ...
    /**
     播放器
     */
    open var player: AVAudioPlayerNode = AVAudioPlayerNode()
    /**
     进度条计时器
     */
    var timer: Timer?
    open var model: MusicModel?
    
    // MARK: - sharedInstance
    @objc
    public static let share = AAudioPlayerModule();
    ...
    open func player(withArr array: [Any], to index: Int, path: String?) {
        self.musicArr = array
        _playIndex = index
        if path == nil {
...
//            _playerUrl = URL(string: model!.savePath);
    ...
    open func seekTotime(_ time: Double) {
        timerOut()
    ...
    open func updateEQ(withBandIndex BandIndex: Int, gain: Float) {
        let filterParams: AVAudioUnitEQFilterParameters? = eq.bands[BandIndex]
        filterParams?.gain = gain
    }
    
    open func updateEQgains(_ gains: [String]?) {
        for i in 0..<9 {
