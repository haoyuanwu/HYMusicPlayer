...
@objc open class FuncTools: NSObject {

    @objc open class func showAlertWarning(title: String,msg:String) {
        let alert = NSAlert()
    ...
    @objc open class func addEqulierAlert(name: String ,complete:((_ returncode :NSApplication.ModalResponse)->Void)?) {
        let alert = NSAlert()
